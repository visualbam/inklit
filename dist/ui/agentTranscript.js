...
export function agentTranscriptTail(text, maxLines) {
    const lines = sanitizeAgentTranscript(text).split("\n");
    return lines.slice(-maxLines).join("\n");
}
export function sanitizeAgentTranscript(text) {
    return text
        .replace(ANSI_RE, "")
        .split(/\r?\n/)
...
function isTerminalChromeLine(line) {
    ...
    if (BARE_PROMPT_RE.test(trimmed))
        return true;
    if (trimmed.length >= 8 && HORIZONTAL_ONLY_RE.test(trimmed))
        return true;
    if (isAgentStatusLine(trimmed))
        return true;
    if (/^scroll\s+J\/K\b.*\?\s+help$/i.test(trimmed))
        return true;
