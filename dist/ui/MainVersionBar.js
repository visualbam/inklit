...
export function MainVersionBar({ mainVersion, targetBranch, tasks, visibleTaskCount, filterQuery, width, }) {
    ...
    const target = targetBranch && targetBranch !== mainVersion.branch
        ? ` · target ${targetBranch}`
        : "";
    const summary = ` · main version ${mainVersion.branch || "unknown"} ${mainVersion.shortSha || "no sha"} ${mainVersion.dirty ? "dirty" : "clean"} · active ${counts.active} ready ${counts.ready} done ${counts.done}${target}${filter} · ${path}`;
