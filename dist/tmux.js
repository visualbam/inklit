...
async function ensureTmuxConf() {
    ...
    if (existing !== conf)
        await fs.writeFile(path, conf, "utf-8");
    return path;
}
 ...
 */
export async function spawnSession(slug, command, args, cwd) {
    const conf = await ensureTmuxConf();
    // -d: start detached  -s: session name  -x/-y: initial terminal size
    await execa("tmux", ["-L", SERVER, "-f", conf, "new-session", "-d", "-s", slug, "-x", "220", "-y", "50", "--", command, ...args], {
        cwd,
        reject: true,
        timeout: 10_000,
    });
...
export async function sessionAlive(slug) {
    try {
 ...
 */
export async function capturePane(slug, opts = {}) {
    const lines = opts.lines ?? 200;
    try {
        const { stdout } = await tmux(["capture-pane", "-t", slug, "-p", "-S", `-${lines}`], { timeout: 1000 });
 ...
 */
export async function sendKeys(slug, text) {
    try {
...
export async function killSession(slug) {
    try {
 ...
 */
export async function openFloat(slug) {
    if (!inSession())
        return null;
    try {
