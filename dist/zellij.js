...
export async function listPanes() {
    if (!inSession())
        return [];
    try {
        const { stdout } = await execa("zellij", ["action", "list-panes", "--json", "--state", "--command", "--tab"], { reject: true, stripFinalNewline: true, timeout: 1000 });
...
export async function findPaneByName(name) {
    const panes = await listPanes();
    return panes.find((p) => (p.name ?? p.title) === name && !p.exited) ?? null;
}
export async function panesSnapshot() {
    const panes = await listPanes();
    const byId = new Map();
 ...
 */
export async function renameOwnPane(name) {
    if (!inSession())
        return;
...
export async function focusPaneId(id) {
    try {
 ...
 */
export async function closePaneById(id) {
    if (!inSession())
        return false;
    try {
 ...
 */
async function discoverStackAnchor() {
    ...
    for (const p of panes) {
        ...
        if (typeof p.id !== "number")
            continue;
        if (home.tab_id !== undefined && p.tab_id !== home.tab_id)
            continue;
        if (!best || (p.id ?? -1) > (best.id ?? -1))
            best = p;
    }
    if (!best)
        return null;
 ...
 */
export async function focusPaneByName(name) {
 ...
 */
export async function dumpScreen(paneId, opts = {}) {
    if (!inSession())
        return "";
 ...
 */
export async function closePaneByName(name) {
    if (!inSession())
        return false;
    const pane = await findPaneByName(name);
    if (!pane)
...
export async function sendKeysToSlug(name, text) {
    if (!inSession())
        return false;
    const pane = await findPaneByName(name);
    if (!pane)
...
export async function sendKeysToPaneId(id, text) {
    if (!inSession())
        return false;
    try {
        if (text) {
            ...
            await new Promise((r) => setTimeout(r, CODEX_BURST_GAP_MS));
        }
...
export async function spawnPane(opts) {
    if (!inSession()) {
        throw new ZellijError("Not in a zellij session. Launch inklit inside zellij so it can spawn panes.");
    }
    const home = ourPaneId();
    ...
    try {
        ...
        const e = err;
        throw new ZellijError(`zellij new-pane failed: ${e.shortMessage ?? e.message}`, typeof e.stderr === "string" ? e.stderr : undefined);
    }
    finally {
        // Refocus inklit so the user keeps interacting with the list.
        if (home)
            await focusPaneId(home);
    }
}
